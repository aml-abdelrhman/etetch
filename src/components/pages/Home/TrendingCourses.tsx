...
const TrendingCourses = () => {
  ...
  const { data: courses, isLoading, error } = useQuery(trendingCoursesQueryOptions());

  if (isLoading) return (
    <div className="flex justify-center py-20"><Loader2 className="w-10 h-10 text-purple-500 animate-spin" /></div>
  );

  if (error) return <div className="py-10 font-bold text-center text-red-500">Error Loading</div>;

  return (
    <section className="container px-5 py-24 mx-auto lg:px-16" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
        {courses?.map((course: any) => {
          ...
          const courseTitle = course.title?.[locale] || course.title?.en || "Course";
          const courseDesc = course.description?.[locale] || course.description?.en || "";

          return (
            <div key={course.id} className="bg-[#211E2E] p-6 rounded-[40px] border border-purple-800/30 transition-all hover:border-purple-500 group flex flex-col items-center text-center">
              ...
              <div className="relative w-40 h-40 rounded-full border-[10px] border-[#00000040] overflow-hidden mb-6 shrink-0">
                <Image 
                  ...
                  alt={courseTitle} // تأكدي إن النص هنا String مش فاضي
                  fill 
                  className="object-cover"
                  unoptimized // جربي دي لو الإيرور لسه موجود
                />
              </div>
              ...
              <button className="w-full mt-auto px-8 py-3 font-bold text-white rounded-full bg-gradient-to-r from-[#8176AF] to-[#BC10D8] uppercase">
                {/* استخدمي h.raw للتأكد إن المفتاح موجود وقيمته مش فاضية */}
